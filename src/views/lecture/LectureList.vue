...
const yearList = computed(() => {
  ...
  return [...new Set(years)].sort((a, b) => b - a); // 내림차순 정렬 (최신 연도가 위로)
});
...
watch(() => route.query, (newQuery) => {
  ...
  if (Object.keys(newQuery).length > 0) {
    ...
    filter.selectedCredits = '';
    filter.selectedMajor = '';
    filter.selectedAcademicYear = '';
    searchInput.value = '';
    searchQuery.value = '';
    // 2. 만약 서버에서 가져온 리스트(yearList)에 오늘 연도가 없다면?
    // 데이터가 있는 연도 중 가장 최신(첫 번째) 연도로 변경
    if (yearList.value.length > 0 && !yearList.value.includes(curYear)) {
